...
import {
  Tv, AlertTriangle, FolderOpen, Rss, RefreshCcw, Play, SearchX, BarChart3,
} from 'lucide-react'
...
export default function DashboardPage() {
  ...
  useWebSocket('scan.failed', (data) => {
    fetchDashboard()
    toast.error(`扫描失败：${data?.error || '未知错误'}`)
  })
  useWebSocket('dashboard.refresh', () => {
    fetchDashboard()
  })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
        <KpiCard
          label="剧集数"
          value={kpi.show_count}
          icon={<Tv size={16} />}
          onClick={() => navigate('/shows')}
        />
        <KpiCard
          label="缺集总数"
          value={kpi.missing_count}
          icon={<AlertTriangle size={16} />}
          onClick={() => navigate('/shows?status=partial')}
        />
        <KpiCard
          label="整季缺失"
          value={kpi.full_missing_count}
          icon={<SearchX size={16} />}
          onClick={() => navigate('/shows?status=full_missing')}
        />
        <KpiCard
          label="已订阅"
          value={kpi.subscribed_count}
          icon={<Rss size={16} />}
          onClick={() => navigate('/subscriptions')}
        />
        <KpiCard
          label="部分缺失"
          value={kpi.partial_count}
          icon={<BarChart3 size={16} />}
          onClick={() => navigate('/shows?status=partial')}
        />
        <KpiCard
          label="未识别"
          value={kpi.unrecognized_count}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-medium">
                {active_scan ? '扫描进度' : '最近扫描'}
              </h2>
              <button
                onClick={handleStartScan}
                disabled={!!active_scan}
              ...
              <div className="mt-4 space-y-3">
                ...
                {active_scan.current_item && (
                  <p className="text-xs text-muted-foreground truncate">
                    当前：{active_scan.current_item}
                  </p>
                )}
                {active_scan.eta_seconds > 0 && (
                  <p className="text-xs text-muted-foreground">
                    预计剩余：{formatDuration(active_scan.eta_seconds)}
                  </p>
                )}
              </div>
            ) : (
          ...
          {recent_scans.length > 0 && (
            <div className="rounded-lg border border-border bg-card p-4">
              <h2 className="mb-3 text-sm font-medium">扫描历史</h2>
                ...
                {recent_scans.map((scan) => (
                  <div key={scan.id} className="flex items-center gap-3 rounded border border-border p-2 text-xs">
                    ...
                    <span className="text-muted-foreground">
                      {scan.source_names.join(', ') || '全部源'}
                    </span>
                    <span className="text-muted-foreground">{scan.show_count} 部剧</span>
                    {scan.duration_seconds && (
                      <span className="text-muted-foreground">{formatDuration(scan.duration_seconds)}</span>
                    )}
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  toast.success(res.data.message || '已刷新')
                  fetchDashboard()
                } catch (e) {
                  toast.error('刷新失败')
                }
